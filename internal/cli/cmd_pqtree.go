...
Example: "0,1" means elements 0 and 1 must be adjacent.`,
  ...
  stacktower pqtree --labels A,B,C,D -o tree.svg 0,1 2,3`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Write SVG output
			if err := writeDataToFile(svg, output); err != nil {
				return err
			}

			// Show summary
			summary := term.NewSuccessSummary("PQ-tree generated")
			summary.AddKeyValue("Tree", tree.StringWithLabels(labelList))
			summary.AddKeyValue("Permutations", fmt.Sprintf("%d", tree.ValidCount()))
			if output != "" {
				summary.AddFile(output)
			}
			summary.Print()
