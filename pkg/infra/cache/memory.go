...
type MemoryCache struct {
	...
	httpCache     map[string]*httpEntry // HTTP response cache
...
func (c *MemoryCache) GetHTTP(ctx context.Context, key string) ([]byte, bool, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()

	entry, ok := c.httpCache[key]
	if !ok {
...
func (c *MemoryCache) SetHTTP(ctx context.Context, key string, data []byte, ttl time.Duration) error {
	c.mu.Lock()
	defer c.mu.Unlock()

	c.httpCache[key] = &httpEntry{
		Data:      data,
...
func (c *MemoryCache) DeleteHTTP(ctx context.Context, key string) error {
	c.mu.Lock()
	defer c.mu.Unlock()

	delete(c.httpCache, key)
	return nil
}
