...
func ExampleWriteGraph() {
	...
	if err := dto.WriteGraph(g, &buf); err != nil {
...
func ExampleReadGraph() {
	...
	// Parse the JSON
	g, err := dto.ReadGraph(bytes.NewReader([]byte(jsonData)))
	if err != nil {
		fmt.Println("Error:", err)
		return
	}
...
func ExampleWriteGraphFile() {
	...
	if err := dto.WriteGraphFile(g, path); err != nil {
		fmt.Println("Error:", err)
		return
	}
...
func ExampleReadGraph_withMetadata() {
	...
	g, _ := dto.ReadGraph(bytes.NewReader([]byte(jsonData)))
