...
func TestToDTO(t *testing.T) {
	...
	if dto.Width != 800 {
		t.Errorf("Width = %v, want 800", dto.Width)
	}
	if dto.Height != 600 {
		t.Errorf("Height = %v, want 600", dto.Height)
	}
	if len(dto.Blocks) != 1 {
		t.Errorf("Blocks count = %d, want 1", len(dto.Blocks))
	}
}

func TestToDTOWithOptions(t *testing.T) {
	l := Layout{
		FrameWidth:  400,
		FrameHeight: 300,
		Blocks:      map[string]Block{},
	...
	if dto.Style != "handdrawn" {
		t.Errorf("Style = %q, want %q", dto.Style, "handdrawn")
	}
	if !dto.Merged {
	...
	if dto.Seed != 12345 {
		t.Errorf("Seed = %d, want 12345", dto.Seed)
	}
}

func TestToDTOWithGraph(t *testing.T) {
	g := dag.New(nil)
	...
	if len(dto.Edges) != 1 {
		t.Errorf("Edges count = %d, want 1", len(dto.Edges))
	}
}
...
func TestRoundTrip(t *testing.T) {
	...
	// Export
	dto, err := original.ToDTO(nil)
	if err != nil {
		t.Fatalf("ToDTO() error: %v", err)
	}

	// Import
	imported, err := FromDTO(dto)
	if err != nil {
		t.Fatalf("FromDTO() error: %v", err)
	}
...
func TestToDTOWithNebraska(t *testing.T) {
	...
	if len(dto.Nebraska) != 1 {
		t.Fatalf("Nebraska count = %d, want 1", len(dto.Nebraska))
	}
...
func TestFromDTOWithNebraska(t *testing.T) {
	input := dto.Layout{
		VizType: dto.VizTypeTower,
		Width:   400,
		Height:  300,
		Blocks:  []dto.Block{},
		Nebraska: []dto.NebraskaRanking{
			{
				Maintainer: "maintainer1",
				Score:      10.5,
				Packages: []dto.NebraskaPackage{
...
func TestToDTOWithBlockMeta(t *testing.T) {
	g := dag.New(nil)
	g.AddNode(dag.Node{
		ID:  "pkg",
		Row: 0,
	...
	if len(dto.Blocks) != 1 {
		t.Fatalf("Blocks count = %d, want 1", len(dto.Blocks))
	}
...
func TestReadBlocksData(t *testing.T) {
	// This test verifies JSON unmarshaling into dto.Layout for blocks.
	input := dto.Layout{
		Width:  400,
		Height: 300,
		Blocks: []dto.Block{
			{ID: "a", Label: "Package A", URL: "https://example.com/a"},
			{ID: "b", Label: "Package B", Meta: &dto.BlockMeta{Stars: 100}},
		},
	}
