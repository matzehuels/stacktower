...
func ExampleNewGitHub() {
	...
	provider := metadata.NewGitHub(artifact.NullBackend{}, token, 24*time.Hour)

	// Use with resolver options
	resolver, _ := python.Language.Resolver(artifact.NullBackend{})
	opts := deps.Options{
...
func ExampleNewComposite() {
	...
	github := metadata.NewGitHub(artifact.NullBackend{}, "", 24*time.Hour)
