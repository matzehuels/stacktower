...
func ResolveAndMerge(ctx context.Context, resolver Resolver, dependencies []Dependency, opts Options) (*dag.DAG, error) {
	...
	// Merge results as they arrive so we don't retain all per-dependency DAGs.
	for res := range results {
		if res.err != nil {
			// Respect cancellation as a hard stop so callers don't receive partial
			// graphs after they explicitly aborted resolution.
